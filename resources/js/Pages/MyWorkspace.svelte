    ...
    <div class="grid items-start gap-6 xl:grid-cols-[minmax(0,1fr)_320px]">
        <div class="min-w-0 space-y-6">
            ...
            {#if allClear}
                ...
                >
                    <p class="font-display text-2xl font-bold tracking-tight text-accent">All clear.</p>
            ...
            {:else}
                <section>
                    <header class="mb-2 flex items-baseline justify-between">
                        <h2 class={`ws-eyebrow ${due.length > 0 ? 'text-danger' : 'text-fg-muted'}`}>
                            ⚠ Due
                        </h2>
                        <span class="font-mono text-[11px] text-fg-muted">{due.length} · next {DUE_WINDOW_DAYS} days</span>
                    </header>

                    {#if due.length === 0}
                        <p class="font-mono text-xs text-fg-muted">✓ Nothing due within {DUE_WINDOW_DAYS} days</p>
                    {:else}
                        <div class="space-y-2">
                            {#each due as a (a.id)}
                ...
                >
                    <header class="mb-2 flex items-baseline justify-between">
                        ...
                        <span class="font-mono text-[11px] text-fg-muted">{focused.length} pinned · drag here to pin</span>
                    </header>
                        ...
                        <div class="space-y-5">
                            {#each othersGrouped as group (group.project.id)}
                                <div>
                                    ...
                                    >
                                        <a href={`/workspace/projects/${group.project.slug}`} class="hover:text-accent"
                                            >{group.project.title}</a
                                        >
                                        <span class="ml-1 text-fg-faint">· {group.assignments.length}</span>
                                    </h3>
                                    <div class="space-y-2">
        ...
        <aside class="space-y-6 xl:sticky xl:top-20">
            ...
            {#if snoozedCount > 0}
                <p class="px-1 font-mono text-[11px] text-fg-muted">💤 {snoozedCount} snoozed</p>
            {/if}
        </aside>
    </div>
</AppShell>
