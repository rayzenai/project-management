...
<main class="flex min-h-screen items-center justify-center bg-bg px-4 py-12 text-fg">
    <div class="w-full max-w-sm">
        ...
        <form onsubmit={submit} class="rounded-2xl border border-line bg-surface p-6 shadow-sm">
            <div class="flex flex-col gap-1.5">
                <label for="{uid}-email" class="text-sm font-medium">Email address</label>
                <input
                    id="{uid}-email"
                    type="email"
                ...
                />
                {#if form.errors.email}<p class="text-xs text-danger">{form.errors.email}</p>{/if}
            </div>

            <div class="mt-4 flex flex-col gap-1.5">
                <label for="{uid}-password" class="text-sm font-medium">Password</label>
                <div class="relative">
                    <input
                        id="{uid}-password"
                ...
                {#if form.errors.password}<p class="text-xs text-danger">{form.errors.password}</p>{/if}
            </div>

            <label class="mt-4 flex items-center gap-2 text-sm text-fg-muted">
                <input type="checkbox" bind:checked={form.remember} class="rounded border-line" />
                Remember me
            </label>
