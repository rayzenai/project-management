<script lang="ts">
    ...
    import { applyAppearance, type Appearance } from '../lib/applyTheme';
    ...
    function normalizeOverride(o: Partial<FontOverride> | null): FontOverride {
        return { display: o?.display ?? null, body: o?.body ?? null, mono: o?.mono ?? null };
    }

    /** Light + dark token sets used for the `system` theme's preview. */
    const systemTokens = $derived.by((): { light: ThemeTokens; dark: ThemeTokens } | undefined => {
    ...
    $effect(() => {
        ...
        let cancelled = false;
        fetch('/api/v1/themes', { headers: { Accept: 'application/json' }, credentials: 'same-origin' })
            .then((r) => r.json())
            .then((json) => {
                if (cancelled) return;
...
<div class="space-y-8">
    ...
    <section class="space-y-3">
        <div>
            <h3 class="font-display text-base font-bold tracking-tight text-fg">Theme</h3>
            <p class="text-sm text-fg-muted">Pick a look. The preview applies instantly.</p>
        </div>

        {#if loading}
            <div class="grid grid-cols-2 gap-3 sm:grid-cols-3">
                {#each Array(6) as _, i (i)}
                    <div class="h-32 animate-pulse rounded-xl border border-line bg-surface-alt"></div>
                {/each}
            </div>
        {:else}
            <div class="grid grid-cols-2 gap-3 sm:grid-cols-3">
                {#each themes as theme (theme.key)}
                    <ThemeCard {theme} selected={selectedKey === theme.key} onselect={() => selectTheme(theme)} />
                {/each}
            </div>
        {/if}
    </section>
    ...
    {#if fontAllowList}
        <section class="space-y-3">
            <div>
                <h3 class="font-display text-base font-bold tracking-tight text-fg">Fonts</h3>
                <p class="text-sm text-fg-muted">Optional. Leave on “Theme default” to follow the theme.</p>
            </div>
            <div class="grid grid-cols-1 gap-3 sm:grid-cols-3">
                {#each fontRoles as { role, label } (role)}
                    <label class="block space-y-1">
                        <span class="font-mono text-[0.7rem] tracking-wide text-fg-faint uppercase">{label}</span>
                        <select
                            value={fontOverride[role] ?? ''}
                            onchange={(e) => changeFont(role, (e.currentTarget as HTMLSelectElement).value)}
    ...
    <!-- Email notifications -->
    <section class="flex items-center justify-between gap-4 rounded-xl border border-line bg-surface p-4">
        <div>
            <h3 class="font-display text-base font-bold tracking-tight text-fg">Email notifications</h3>
            <p class="text-sm text-fg-muted">Receive workspace updates by email.</p>
        </div>
        <button
            type="button"
            role="switch"
