...
<form onsubmit={submit} class="mt-3 rounded-xl border border-line bg-surface p-4">
    ...
    <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
        <div>
            ...
            />
            {#if form.errors.title}<p class="mt-1 text-xs text-danger">{form.errors.title}</p>{/if}
        </div>
        <div>
            ...
            <input
                ...
                class="w-full rounded-md border border-line bg-surface px-3 py-1.5 text-sm text-fg"
            />
            {#if form.errors.title_np}<p class="mt-1 text-xs text-danger">{form.errors.title_np}</p>{/if}
        </div>
    </div>
    ...
    <div class="mt-3">
        ...
        <textarea
            ...
            class="w-full rounded-md border border-line bg-surface px-3 py-1.5 text-sm text-fg"
        ></textarea>
        {#if form.errors.description}<p class="mt-1 text-xs text-danger">{form.errors.description}</p>{/if}
    </div>

    <div class="mt-3">
        <label for={`${uid}-description-np`} class="mb-1 block text-xs font-medium text-fg-muted">Description (Nepali)</label>
        <textarea
            ...
            class="w-full rounded-md border border-line bg-surface px-3 py-1.5 text-sm text-fg"
        ></textarea>
        {#if form.errors.description_np}<p class="mt-1 text-xs text-danger">{form.errors.description_np}</p>{/if}
    </div>
