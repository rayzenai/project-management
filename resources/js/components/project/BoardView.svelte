<script lang="ts">
    ...
    let {
        ...
        statuses,
        filtersActive = false,
    }: { project: Project; tasks: Task[]; statuses: Status[]; filtersActive?: boolean } = $props();
    ...
    let pendingMove = $state<{ taskId: number; toStatus: string; orderedIds: number[] } | null>(null);
    ...
    const columnDefs = $derived.by(() => {
        const defs = statuses.map((s) => ({ value: s.value, label: s.label, color: s.color }));
        if (tasks.some((t) => !knownValues.has(t.status))) defs.push({ value: OTHER, label: 'Other', color: '#9CA3AF' });
        return defs;
    });

    const columns = $derived.by(() => {
        const map = new SvelteMap<string, Task[]>();
        for (const def of columnDefs) map.set(def.value, []);
        for (const t of tasks) map.get(knownValues.has(t.status) ? t.status : OTHER)?.push(t);
        ...
        if (pm) {
            ...
            if (moved) {
                ...
                for (const t of map.get(pm.toStatus) ?? []) pool.set(t.id, t);
                ...
                map.set(
                    pm.toStatus,
                    pm.orderedIds.map((id) => pool.get(id)).filter((t): t is Task => t !== undefined),
                );
            }
        }
        return map;
    ...
    function onDragStart(task: Task, event: DragEvent) {
        ...
        event.dataTransfer?.setData('application/x-pm-task', JSON.stringify({ id: task.id, fromStatus: task.status }));
    ...
    function unchangedOrder(orderedIds: number[], status: string): boolean {
        ...
        return current.length === orderedIds.length && current.every((id, i) => id === orderedIds[i]);
    }
    ...
    function onDrop(toStatus: string, event: DragEvent) {
        ...
        const index = target?.status === toStatus ? target.index : column.length;

        const orderedIds = column.map((t) => t.id).filter((tid) => tid !== id);
        orderedIds.splice(Math.min(index, orderedIds.length), 0, id);
        if (fromStatus === toStatus && unchangedOrder(orderedIds, toStatus)) return; // no-op drop
                ...
                {:else}
                    {#each colTasks as task, index (task.id)}
                        ...
                        <BoardCard
                            ...
                            ondragend={onDragEnd}
                            ondragover={droppable ? (e) => onCardDragOver(def.value, index, e) : () => {}}
                        />
                    {/each}
            ...
            {#if droppable && !(filtersActive && colTasks.length === 0)}
                <div class="px-2 pb-3">
                    {#if composerOpenFor === def.value}
                        <ColumnComposer {project} status={def.value} onClose={() => (composerOpenFor = null)} />
                    {:else}
                        <button
                            type="button"
