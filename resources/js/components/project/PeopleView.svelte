...
<div class="space-y-4">
    {#if onlyUnassigned}
        <p class="text-sm text-fg-muted">No one is assigned yet — open a task and add assignees from the Peek.</p>
    {/if}
        ...
        <section class="rounded-xl border border-line bg-surface">
            <header class="flex items-center gap-3 px-4 py-3">
                {#if bucket.member}
                    ...
                    <h3 class="min-w-0 flex-1 truncate text-sm font-semibold text-fg">{bucket.member.name}</h3>
                {:else}
                    <span
                    ...
                    <h3 class="min-w-0 flex-1 truncate text-sm font-semibold text-fg-muted">Unassigned</h3>
                {/if}
            ...
            {#if !collapsed.has(bucket.key)}
                <div class="border-t border-line-soft">
                    {#if bucket.tasks.length === 0}
                        <p class="px-4 py-3 text-sm text-fg-muted">All done ✓</p>
                    {:else}
