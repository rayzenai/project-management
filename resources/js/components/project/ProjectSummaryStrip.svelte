<script lang="ts">
    ...
    const isOverdue = (t: Task) => !!t.deadline_at && !isComplete(t) && new Date(t.deadline_at) < new Date(new Date().toDateString());
    ...
    const percent = $derived(total === 0 ? 0 : Math.round((doneCount / total) * 100));
    const overdueCount = $derived(tasks.filter(isOverdue).length);
    // Incomplete only — done-and-unassigned isn't actionable.
    const unassignedCount = $derived(tasks.filter((t) => !isComplete(t) && (t.assignments ?? []).length === 0).length);

    const segments = $derived(
        statuses
            .map((s) => ({ value: s.value, label: s.label, color: s.color, count: tasks.filter((t) => t.status === s.value).length }))
            .filter((s) => s.count > 0),
    );
</script>
