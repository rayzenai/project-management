<script lang="ts">
    ...
    const isComplete = $derived(statuses.find((s) => s.value === shown)?.is_complete ?? false);
    const firstIncomplete = $derived(statuses.find((s) => !s.is_complete)?.value ?? 'not_started');

    $effect(() => {
        if (optimistic !== null && task.status === optimistic) optimistic = null;
    });

    function patchStatus(value: string, options: { onSuccess?: () => void } = {}) {
