<script lang="ts">
    ...
    type ProjectResult = { id: number; slug: string; title: string; tasks_count: number };
    type TaskResult = {
        id: number;
        slug: string;
    ...
    type TaskRef = { id: number; slug: string; title: string; project: { slug: string; title: string } | null } | null;
    ...
    type SearchResults = { projects: ProjectResult[]; tasks: TaskResult[]; notes: NoteResult[]; contacts: ContactResult[] };

    type PaletteItem =
        | { kind: 'action'; id: 'new-task'; label: string }
        | { kind: 'nav'; href: string; label: string; icon: string }
        | { kind: 'project'; id: number; slug: string; title: string; meta: string }
        | { kind: 'task'; id: number; slug: string; title: string; item_number: number | null; meta: string }
        | { kind: 'note'; noteKind: 'task' | 'sticky'; id: number; body: string; task: { id: number; slug: string } | null; meta: string }
        | { kind: 'contact'; id: number; name: string; meta: string; task: { id: number; slug: string } | null };

    const EMPTY: SearchResults = { projects: [], tasks: [], notes: [], contacts: [] };
    ...
    const grouped = $derived.by<{ label: string; items: PaletteItem[]; offset: number }[]>(() => {
        const q = query.trim();
        ...
        groups.push({
            label: 'Actions',
            items: [{ kind: 'action', id: 'new-task', label: q ? `New task… "${q}"` : 'New task…' }],
        });

        const nav = q ? NAV.filter((n) => n.label.toLowerCase().includes(lq)) : NAV;
        if (nav.length) {
            groups.push({ label: 'Navigate', items: nav.map((n) => ({ kind: 'nav' as const, href: n.href, label: n.label, icon: n.icon })) });
        }
            ...
            if (results.projects.length) {
                groups.push({
                    ...
                    items: results.projects
                        .slice(0, 5)
                        .map((p) => ({ kind: 'project' as const, id: p.id, slug: p.slug, title: p.title, meta: `${p.tasks_count} tasks` })),
                });
            }
            if (results.tasks.length) {
                groups.push({
                    ...
                    items: results.tasks.slice(0, 8).map((t) => ({
                        ...
                        meta: [t.project?.title, t.status_label].filter(Boolean).join(' · '),
                    })),
                });
            }
            if (results.notes.length) {
                groups.push({
                    ...
                    items: results.notes.slice(0, 4).map((n) => ({
                        ...
                        id: n.id,
                        body: n.body,
                        task: n.task ? { id: n.task.id, slug: n.task.slug } : null,
                        meta: n.task?.title ?? (n.kind === 'sticky' ? 'Sticky note' : ''),
                    })),
                });
            }
            if (results.contacts.length) {
                groups.push({
                    ...
                    items: results.contacts.slice(0, 4).map((c) => ({
                        ...
                        id: c.id,
                        name: c.name,
                        meta: [c.role, c.organization].filter(Boolean).join(' · '),
                        task: c.task ? { id: c.task.id, slug: c.task.slug } : null,
                    })),
                });
            }
        }
    ...
    const flat = $derived<PaletteItem[]>(grouped.flatMap((group) => group.items));
    ...
    $effect(() => {
        ...
        if (!palette.isOpen || typeof document === 'undefined') return;
        document.getElementById(`pal-item-${idx}`)?.scrollIntoView({ block: 'nearest' });
    });
