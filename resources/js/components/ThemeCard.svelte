<script lang="ts">
    ...
    const c = $derived(theme.key === 'system' ? SYSTEM_PREVIEW.color : theme.tokens!.color);
    const f = $derived(theme.key === 'system' ? SYSTEM_PREVIEW.font : theme.tokens!.font);
</script>
...
>
    ...
    <div class="p-3" style={`background:${c.bg}`}>
        <div class="rounded-lg p-2.5" style={`background:${c.surface};border:1px solid ${c.line}`}>
            <div class="mb-2 flex items-center gap-2">
                <span class="h-2.5 w-2.5 rounded-full" style={`background:${c.accent}`}></span>
                <span
                    class="text-[0.7rem] font-bold tracking-tight"
                    style={`color:${c.text};font-family:${f.display}`}>{theme.label}</span
                >
            </div>
            <div class="space-y-1.5">
                <div class="h-1.5 w-full rounded-full" style={`background:${c.surfaceAlt}`}></div>
                <div class="h-1.5 w-3/4 rounded-full" style={`background:${c.line}`}></div>
                <div class="flex items-center gap-1.5 pt-0.5">
                    <span class="h-1.5 w-1.5 rounded-full" style={`background:${c.accent}`}></span>
                    <span class="text-[0.55rem]" style={`color:${c.textMuted};font-family:${f.body}`}
                        >Aa preview</span
                    >
                </div>
            </div>
    ...
    >
        <span class="text-xs font-medium" style={`color:${c.text};font-family:${f.body}`}>{theme.label}</span>
        {#if selected}
            <span
