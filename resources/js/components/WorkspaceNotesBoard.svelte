<script lang="ts">
    ...
    $effect(() => {
        const measure = () => (viewport = { w: window.innerWidth, h: window.innerHeight });
        measure();
    ...
    function scatter(id: number): { x: number; y: number } {
        ...
        return {
            x: TASK_MARGIN + rand(12.9898) * Math.max(0, viewport.w - TASK_CARD_W - TASK_MARGIN * 2),
            y: 84 + rand(78.233) * Math.max(0, viewport.h - TASK_CARD_H - 84 - TASK_MARGIN),
        };
    }
    ...
    const arranged = $derived.by((): { label: string; items: ArrangeItem[] }[] => {
        if (arrange === 'free') return [];

        const items: ArrangeItem[] = [
            ...notes.map((n) => ({ key: `w-${n.id}`, workspace: n })),
            ...taskNotes.map((n) => ({ key: `t-${n.id}`, task: n })),
        ];
        const text = (it: ArrangeItem) => (it.workspace ? it.workspace.title || it.workspace.body : (it.task?.body ?? '')).toLowerCase();

        if (arrange === 'title') {
            return [{ label: '', items: [...items].sort((a, b) => text(a).localeCompare(text(b))) }];
        }

        const groups = new Map<string, ArrangeItem[]>();
        for (const it of items) {
            const label =
                arrange === 'task'
                    ? it.task
                        ? it.task.task?.short_title || it.task.task?.title || 'Untagged'
                        : 'My notes'
                    : it.task
                      ? it.task.type_label
                      : 'Sticky note';
    ...
    let taskPos = $state<Record<number, { x: number; y: number }>>(loadTaskPos());
    ...
    function beginTaskDrag(note: Note, e: PointerEvent) {
        ...
        if ((e.target as HTMLElement)?.closest('[data-task-link]')) return;
    ...
    function onPointerMove(e: PointerEvent) {
        ...
        if (taskDragId !== null) {
            ...
            if (Math.abs(dx) + Math.abs(dy) > 4) taskMoved = true;
            taskPos = { ...taskPos, [taskDragId]: { x: clampX(taskOriginX + dx), y: clampY(taskOriginY + dy) } };
        }
    }
...
{#if open}
    <div class="fixed inset-0 z-50 bg-black/30 backdrop-blur-[2px] select-none">
        ...
        <button type="button" aria-label="Close notes board" class="absolute inset-0 h-full w-full cursor-default" onclick={onClose}></button>
        ...
        <div class="pointer-events-none absolute inset-x-0 top-0 z-10 flex items-center justify-between px-4 py-3">
            ...
            >
                ...
                <span>My notes</span>
                <span class="bg-surface-alt text-fg-muted rounded-full px-2 py-0.5 text-xs font-medium">{notes.length + taskNotes.length}</span>
            </div>
            <div class="pointer-events-auto flex items-center gap-2">
                <label class="bg-surface/90 flex items-center gap-1.5 rounded-full py-1.5 pr-2 pl-3 text-sm shadow-sm ring-1 ring-line">
                    <span class="text-fg-faint text-xs font-medium">Arrange</span>
                    <select
                        bind:value={arrange}
                ...
                <button
                    ...
                    class="bg-accent text-bg hover:bg-accent-dim rounded-full px-4 py-1.5 text-sm font-semibold shadow-sm">+ New note</button
                >
                <button
                    type="button"
                    onclick={onClose}
                    aria-label="Close"
                    class="bg-surface/90 text-fg-muted hover:text-fg rounded-full p-2 shadow-sm ring-1 ring-line">✕</button
                >
            </div>
        </div>
        ...
        {#if notes.length === 0 && taskNotes.length === 0 && !composing}
            <div class="pointer-events-none absolute inset-0 flex items-center justify-center">
                <p class="bg-surface/80 text-fg-muted rounded-xl px-4 py-2 text-sm shadow-sm">No notes yet — tap “+ New note” to pin one.</p>
            </div>
        {/if}
        ...
        {#if arrange === 'free'}
            ...
            {#each taskNotes as note (`t-${note.id}`)}
                ...
                <div
                    ...
                    style:transition={isTaskDragging ? 'none' : 'transform 150ms ease'}
        ...
        {:else}
            ...
            <div class="absolute inset-x-0 top-16 bottom-0 overflow-y-auto px-4 pb-6">
                <div class="pointer-events-auto mx-auto max-w-5xl space-y-6">
                    {#each arranged as group (group.label || 'all')}
                        <div>
                            {#if group.label}
                                <div class="mb-2 flex items-center gap-2">
                                    <h3 class="ws-eyebrow text-fg-muted">{group.label}</h3>
                                    <span class="bg-surface-alt text-fg-muted rounded-full px-1.5 py-0.5 text-[10px] font-medium"
                            ...
                            <div class="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
                                {#each group.items as it (it.key)}
                                    {#if it.workspace}
                                        ...
                                        <div class={`flex h-36 flex-col rounded-md border p-3 shadow-sm ${paperClass[note.color]}`}>
                                            {#if note.title}
                                                <span class="mb-0.5 line-clamp-1 text-sm font-bold text-neutral-900 dark:text-neutral-50"
                                                    >{note.title}</span
                                                >
                                            {/if}
                                            <span
                                                ...
                                                >{note.body}</span
                                            >
                                            <div class="mt-1.5 flex items-center justify-between border-t border-black/10 pt-1 dark:border-white/10">
                                                <span class="text-[10px] text-neutral-600/70 dark:text-neutral-300/60"
                                                    >{formatDate(note.updated_at)}</span
                                                >
                                                <div class="flex items-center gap-1">
                                                    <button
                                                        type="button"
                                                        onclick={() => openWorkspaceNote(note)}
                                    ...
                                    {:else if it.task}
                                        ...
                                        >
                                            ...
                                            <span class="line-clamp-4 flex-1 text-xs leading-snug text-neutral-800 dark:text-neutral-100"
                                                >{note.body}</span
                                            >
                                            {#if note.task}
                                                <span class="mt-1 line-clamp-1 text-[10px] font-medium text-neutral-700/70 dark:text-neutral-200/70">
                                                    on: {note.task.short_title || note.task.title}
                                                </span>
                                            {/if}
                                        </svelte:element>
        ...
        {#if composing}
            <div class="absolute inset-0 z-50 flex items-center justify-center p-4">
                <div class="bg-surface w-full max-w-sm rounded-xl border border-line p-4 shadow-2xl">
                    <div class="mb-2 flex items-center justify-between">
                        ...
                        <button
                            ...
                            aria-label="Cancel"
                            class="text-fg-faint hover:bg-surface-alt hover:text-fg rounded-md p-1">✕</button
                        >
                    </div>
                    <form
                        onsubmit={(e) => {
                    ...
                    >
                        ...
                        <div class="flex justify-end gap-2">
                            <button
                                type="button"
                                onclick={() => (composing = false)}
                                class="text-fg-muted hover:bg-surface-alt rounded-lg px-3 py-1.5 text-sm font-medium">Cancel</button
                            >
                            <button
                                type="submit"
                                disabled={!draftBody.trim() || saving}
