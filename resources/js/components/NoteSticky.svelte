<script lang="ts">
    ...
    const color = $derived(props.kind === 'task' ? noteTypeColor(props.note.type) : (props.note as WorkspaceNote).color);
...
{#if props.kind === 'freeform'}
    ...
    >
        {#if props.note.title}
            <span class="line-clamp-1 text-xs font-bold text-neutral-900 dark:text-neutral-50">{props.note.title}</span>
        {/if}
        <span class="line-clamp-3 text-[11px] leading-snug text-neutral-800 dark:text-neutral-100">{props.note.body}</span>
    </button>
{:else}
    {@const href = taskHref}
    <svelte:element
    ...
    >
        ...
        <span class="line-clamp-2 flex-1 text-[11px] leading-snug text-neutral-800 dark:text-neutral-100">{props.note.body}</span>
        {#if props.note.task}
