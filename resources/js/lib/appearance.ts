...
export const THEME_ORDER = ['system', 'terminal-noir', 'light', 'dark', 'paper', 'glass'];

/** Turns the `themes` map from `GET /api/v1/themes` into an ordered array. */
export function themesToList(themes: Record<string, { label: string; mode: 'light' | 'dark' | null; tokens?: ThemeTokens }>): ConfigTheme[] {
    const keys = Object.keys(themes);
    const ordered = [...THEME_ORDER.filter((k) => k in themes), ...keys.filter((k) => !THEME_ORDER.includes(k))];

    return ordered.map((key) => ({ key, label: themes[key].label, mode: themes[key].mode, tokens: themes[key].tokens }));
}
 ...
 */
export function previewAppearance(theme: ConfigTheme, override: FontOverride, systemTokens?: { light: ThemeTokens; dark: ThemeTokens }): Appearance {
