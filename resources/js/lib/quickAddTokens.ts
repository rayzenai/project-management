...
export type SegmentType = 'plain' | 'project' | 'assignee' | 'priority' | 'date';
...
const RULES: { type: TokenType; re: RegExp }[] = [
    ...
    { type: 'priority', re: /(?<=^|\s)!(?:low|medium|high|urgent|p[1-4])(?=\s|$)/gi },
    {
        type: 'date',
        re: new RegExp(
            `(?<=^|\\s)(?:today|tomorrow|${WEEKDAYS}` + `|\\d{4}-\\d{2}-\\d{2}` + `|(?:${MONTHS})\\s+\\d{1,2}|\\d{1,2}\\s+(?:${MONTHS}))(?=\\s|$)`,
            'gi',
        ),
    },
];
...
export function tokenize(input: string): Segment[] {
    ...
    for (const rule of RULES) {
        ...
        for (const m of input.matchAll(rule.re)) {
            ...
            if (matches.some((t) => start < t.end && end > t.start)) continue;
    ...
    for (const t of matches) {
        if (t.start > cursor) segments.push({ text: input.slice(cursor, t.start), type: 'plain' });
        segments.push({ text: input.slice(t.start, t.end), type: t.type });
        cursor = t.end;
    }
    if (cursor < input.length) segments.push({ text: input.slice(cursor), type: 'plain' });
