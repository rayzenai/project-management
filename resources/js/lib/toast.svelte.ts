...
function show(
    message: string,
    options: { variant?: ToastVariant; undo?: { label?: string; run: () => void | Promise<void> }; duration?: number } = {},
): number {
    const id = nextId++;
    ...
    items = [
        ...items,
        {
            ...
            undo: options.undo ? { label: options.undo.label ?? 'Undo', run: options.undo.run } : null,
        },
    ];
