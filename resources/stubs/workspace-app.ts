...
import { applyAppearance, type Appearance } from '../../../vendor/rayzenai/project-management/resources/js/lib/applyTheme';
...
createInertiaApp({
    resolve: (name) => {
        const pages = import.meta.glob<ResolvedComponent>('../../../vendor/rayzenai/project-management/resources/js/Pages/**/*.svelte', { eager: true });
        return pages[`../../../vendor/rayzenai/project-management/resources/js/Pages/${name}.svelte`];
    },
    setup({ el, App, props }) {
        if (!el) {
            return;
